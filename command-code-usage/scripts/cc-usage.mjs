...
// 本仓库里这个脚本是**按需**跑的（ZCode 的 /quota、/usage），没有每轮消息都起的宿主钩子，
// 所以 --serve 这条常驻路径留在这里没有代价。启动开销仍然是体验本身，所以：
// ESM 静态 import 每个内建模块约 +2ms，改走 createRequire 实测省 11ms。
// node:crypto 不用了；node:http 只有 --serve 需要，因此在 serve() 里惰性取——
// 省掉的不是代码，是其它每一种模式都要付的启动时间。
...
/**
 * --serve：把面板挂在一个只绑回环地址的小 HTTP 服务上，浏览器每 30s 拉一次。
 * 页面里的数字和命令面板完全同源（同一个 collectUsage → normalize → renderHtml），
 * 所以这里不重复任何取数或渲染逻辑。
 */
async function serve(creds, opts) {
  // 惰性取 node:http：只有这条路径用得到，顶层 import 会让 --compact / --statusline
  // 这些每轮都跑的模式一起付加载开销（见文件开头的说明）。
