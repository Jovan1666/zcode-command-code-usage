...
for (const c of [...byName.values()].sort((a, b) => a.name.localeCompare(b.name))) {
  ...
  // 用户级安装会把占位符替换成绝对路径；走市场安装时它保持原样，改由
  // $ZCODE_PLUGIN_ROOT / 已知目录探测解析——两种情况都要如实报出来。
  if (m && !m[1].includes('@@')) {
    console.log(`      脚本路径       = ${m[1]}  ${fs.existsSync(m[1]) ? '✓ 存在' : '✗ 不存在'}`);
  } else {
    console.log('      脚本路径       = 未注入（运行时按 $ZCODE_PLUGIN_ROOT → 已知目录探测）');
  }
}
